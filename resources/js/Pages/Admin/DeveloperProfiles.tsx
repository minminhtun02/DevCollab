...
export default function DeveloperProfiles() {
    ...
    return (
        <AuthGuard portal="admin" loginPath="/admin/login">
            <AppShell portal="admin" navItems={navItems} title="Developer Profiles">
                <PageHeader title="Developer Profiles" />
                <DataTable
                    fetchFunction={fetchProfiles}
                    columns={[
                        { key: 'name', header: t.auth.name, render: (row) => row.user?.name ?? '—' },
